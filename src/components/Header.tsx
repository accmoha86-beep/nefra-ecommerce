    ...
    <header className="header" dir={currentLang.direction}>
      <div className="header-inner">
        ...
        <div className="header-actions">
          ...
          {ff['ff_compare'] !== false && <button className="header-btn" onClick={() => setPage('compare')} title={t('compare')}>
            <Scale size={18}/>
            {compareCount > 0 && <span className="cart-count">{compareCount}</span>}
          </button>}
          {ff['ff_wishlist'] !== false && <button className="header-btn" onClick={() => requireAuth ? requireAuth('wishlist') : setPage('wishlist')} title={t('wishlist')}>
            <Heart size={18}/>
      ...
      <div className={`mobile-nav-drawer${mobileMenuOpen ? ' active' : ''}`}>
        ...
        {ff['ff_wishlist'] !== false && <button onClick={() => {requireAuth ? requireAuth('wishlist') : setPage('wishlist');setMobileMenuOpen(false);}}><Heart size={16} style={{marginInlineEnd:'8px'}}/>{t('wishlist')} {wishlistCount > 0 && <span className="badge-sm">{wishlistCount}</span>}</button>}
        {ff['ff_compare'] !== false && <button onClick={() => {setPage('compare');setMobileMenuOpen(false);}}><Scale size={16} style={{marginInlineEnd:'8px'}}/>{t('compare')} {compareCount > 0 && <span className="badge-sm">{compareCount}</span>}</button>}
