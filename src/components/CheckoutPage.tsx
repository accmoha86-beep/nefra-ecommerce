...
export const CheckoutPage: React.FC<CheckoutPageProps> = ({ lang, cart, setPage, t, formatPrice, currentCountry, featureFlags = [] }) => {
  ...
  const codFeeAmount = Math.round(total * (currentCountry.codFeePercent || 2) / 100);
              ...
              {paymentMethod === 'instapay' && (
                <div className="payment-instructions instapay-box">
                  ...
                  <div className="pi-detail-row">
                    ...
                    <span className="pi-value">{currentCountry.paymentAccountDetails?.instapay?.ipaAddress || 'nefra@instapay'}</span>
                    <button className="copy-btn" onClick={() => copyToClipboard(currentCountry.paymentAccountDetails?.instapay?.ipaAddress || 'nefra@instapay', 'ipa')}>
                ...
                <div className="payment-instructions bank-box">
                  ...
                  <div className="pi-details">
                    <div className="pi-detail-row">
                      ...
                      <span className="pi-value">{currentCountry.paymentAccountDetails?.bankTransfer?.bankName || 'CIB'}</span>
                    </div>
                    <div className="pi-detail-row">
                      <span className="pi-label">{t('accountName')}</span>
                      <span className="pi-value">{currentCountry.paymentAccountDetails?.bankTransfer?.accountName || 'NEFRA Store'}</span>
                    </div>
                    <div className="pi-detail-row">
                      <span className="pi-label">{t('accountNumber')}</span>
                      <span className="pi-value">{currentCountry.paymentAccountDetails?.bankTransfer?.accountNumber || ''}</span>
                      <button className="copy-btn" onClick={() => copyToClipboard(currentCountry.paymentAccountDetails?.bankTransfer?.accountNumber || '', 'acct')}>
                        {copied === 'acct' ? <Check size={14}/> : <Copy size={14}/>}
                      </button>
                    </div>
                    <div className="pi-detail-row">
                      <span className="pi-label">IBAN</span>
                      <span className="pi-value">{currentCountry.paymentAccountDetails?.bankTransfer?.iban || ''}</span>
                      <button className="copy-btn" onClick={() => copyToClipboard(currentCountry.paymentAccountDetails?.bankTransfer?.iban || '', 'iban')}>
              ...
              {paymentMethod === 'vodafone' && (
                <div className="payment-instructions vodafone-box">
                  ...
                  <div className="pi-detail-row">
                    ...
                    <span className="pi-value">{currentCountry.paymentAccountDetails?.vodafoneCash?.phoneNumber || ''}</span>
                    <button className="copy-btn" onClick={() => copyToClipboard(currentCountry.paymentAccountDetails?.vodafoneCash?.phoneNumber || '', 'vf')}>
              ...
              {paymentMethod === 'cod' && (
                <div className="payment-instructions cod-box">
                  ...
                  <div className="pi-detail-row">
                    ...
                    <span className="pi-value">{formatPrice(codFeeAmount)} ({currentCountry.codFeePercent || 2}%)</span>
                  </div>
                  <div className="pi-detail-row">
